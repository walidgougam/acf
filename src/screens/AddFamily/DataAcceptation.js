...
import {addNewFamily} from '../../lib/sync';
import uuid from 'uuid';

export default class DataAcceptation extends Component {
  constructor(props) {
    super(props);
    this.state = {
      agreeState1: false,
  ...
  btnNext = () => {
    ...
    NetInfo.fetch().then(async state => {
      familyData.uuid = uuid();
      if (state.isConnected) {
        await addNewFamily(familyData, membersFamily, food_activity);
      } else {
