...
export default class DeliverAndHistorySearch extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: true,
    ...
    database
      ...
      .once('value', snap => {
        ...
        for (let i = 0; i < newSnapshot.length; i++) {
          if (
            newSnapshot[1][i] &&
            newSnapshot[1][i] !== undefined &&
            newSnapshot[1][i].toLowerCase() === this.state.healthArea
          ) {
            this.setState({goodHealthArea: true});
          }
        }
      });
  };
